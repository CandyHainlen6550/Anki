...
hintBtn.addEventListener('click',function(){if(idx>=paths.length)return;hints.innerHTML='';var p=E('path',pathAttrs(paths[idx],idx));p.style.opacity='.35';hints.appendChild(p);try{var L=p.getTotalLength();p.style.strokeDasharray=L;p.style.strokeDashoffset=L;p.style.transition='stroke-dashoffset .55s ease';requestAnimationFrame(function(){p.style.strokeDashoffset='0'})}catch(e){}setTimeout(function(){try{hints.innerHTML=''}catch(e){}},900)});
